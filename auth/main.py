...
''' Finland JOBS Data API '''

@app.get("/indeed-fi/")
async def indeed_fi_jobs(offset: int, keyword: str, location: str, api_key: APIKey = Depends(auth.get_api_key)):
    len_of_chars = 8
    filename = ''.join(random.choices(string.ascii_uppercase + string.digits, k=len_of_chars))
    ...
    print(location)
    # scrapy_runner.indeed_us(offset, keyword, location, full_name)
    scrapy_runner.indeed_fi(offset, keyword, location, full_name)
    file_size = os.path.getsize(f"{full_name}")
    print("Size of File", file_size)
    if file_size == 0:
        json_str = json.dumps({
...
''' Sweden JOBS Data API '''

@app.get("/indeed-se/")
async def indeed_se_jobs(offset: int, keyword: str, location: str, api_key: APIKey = Depends(auth.get_api_key)):
    len_of_chars = 8
    filename = ''.join(random.choices(string.ascii_uppercase + string.digits, k=len_of_chars))
    ...
    print(location)
    # scrapy_runner.indeed_us(offset, keyword, location, full_name)
    scrapy_runner.indeed_se(offset, keyword, location, full_name)
    file_size = os.path.getsize(f"{full_name}")
    print("Size of File", file_size)
    if file_size == 0:
        json_str = json.dumps({
...
''' Denmark JOBS Data API '''

@app.get("/indeed-dk/")
async def indeed_dk_jobs(offset: int, keyword: str, location: str, api_key: APIKey = Depends(auth.get_api_key)):
    len_of_chars = 8
    filename = ''.join(random.choices(string.ascii_uppercase + string.digits, k=len_of_chars))
    ...
    print(location)
    # scrapy_runner.indeed_us(offset, keyword, location, full_name)
    scrapy_runner.indeed_dk(offset, keyword, location, full_name)
    file_size = os.path.getsize(f"{full_name}")
    print("Size of File", file_size)
    if file_size == 0:
        json_str = json.dumps({
